...
conn = pyodbc.connect('DRIVER={ODBC Driver 17 for SQL Server};SERVER=DESKTOP-LHB569H\SQLEXPRESS;DATABASE=botdb;Trusted_Connection=yes')
cursor = conn.cursor()
...
async def start_cmd_handler(message: types.Message):
    ...
    cursor.execute("INSERT INTO users (telegram_id, chat_id, first_name, last_name, username) VALUES (?, ?, ?, ?, ?)", message.from_user.id, message.chat.id, message.from_user.first_name, message.from_user.last_name, message.from_user.username)
    conn.commit()
...
async def register_meter_cmd_handler(message: types.Message):
    ...
    cursor.execute("INSERT INTO meters (user_id, type, serial_number, location) VALUES ((SELECT id FROM users WHERE telegram_id = ?), ?, ?, ?)", message.from_user.id, meter_type, serial_number, location)
    conn.commit()

    await message.answer(f"Прибор учета {meter_type} зарегистрирован: серийный номер {serial_number}, расположение {location}")
